<?php // لا توجد مسافات قبل هذا السطر
// تصحيح المسار: الصعود بمستوى واحد للأعلى من /my إلى /public_html
require_once dirname(__DIR__) . '/includes/bootstrap.php';

// تأكد من تحميل ملف functions.php
...
/* ----------------------------------------------------------
   التحقق من تسجيل الدخول - القاعدة الموحدة
---------------------------------------------------------- */
$isLoggedIn = !empty($_SESSION['user']) || !empty($_SESSION['user_id']) || !empty($_SESSION['is_member_logged']);

// الحصول على بيانات المستخدم من الجلسة بشكل موحد
$user = null;
$userId = 0;

if ($isLoggedIn) {
    // بناء كائن المستخدم الموحد
    $user = [];
    ...
    // محاولة الحصول على البريد الإلكتروني
...
if ($pdo instanceof PDO && $userId > 0) {
    try {
        ...
        // دمج بيانات الجلسة مع بيانات قاعدة البيانات
        if (!empty($userData)) {
...
/* ----------------------------------------------------------
   معالجة النماذج (تحديث الملف الشخصي)
---------------------------------------------------------- */
$successMessage = '';
$errorMessage = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (!hash_equals($csrfToken, $postedToken)) {
        ...
        if ($action === 'update_profile' && $pdo instanceof PDO) {
            ...
            // التحقق من عدم تكرار البريد الإلكتروني (إذا تغير)
...
// تفعيل التبويبات باستخدام URL hash
