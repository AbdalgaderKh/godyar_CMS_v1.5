...
if (!function_exists('h')) {
    /** Escape for HTML body/text context */
    function h($v): string {
        return htmlspecialchars((string)$v, ENT_QUOTES, 'UTF-8');
    }
}
if (!function_exists('h_attr')) {
    /** Escape for HTML attribute context (same as h, kept for clarity) */
    function h_attr($v): string {
        return htmlspecialchars((string)$v, ENT_QUOTES, 'UTF-8');
    }
}
if (!function_exists('h_url')) {
    /** Best-effort URL escape (does not validate scheme); use for query values */
