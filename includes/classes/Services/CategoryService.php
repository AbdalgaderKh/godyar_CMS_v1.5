...
final class CategoryService
{
    ...
    public function listPublishedNews(int $categoryId, int $page = 1, int $perPage = 12, string $sort = 'latest', string $period = 'all'): array
    {
        ...
        // Primary filter: category + published
        ...
        try {
            ...
            // If schema differs (shared hosting / older DB), the strict publishedWhere() may reference
            // non-existent columns. Don't break the category page: fall back to a simpler query.
            error_log('[CategoryService] listPublishedNews count error: ' . $e->getMessage());
            $fallback = true;
        }

        // If strict filter returns nothing, but there are items in this category,
        // fall back to a simpler condition so the page doesn't appear empty.
        if ($total === 0 && !$fallback) {
            try {
                $simpleWhere = 'n.category_id = :cid';
                if ($this->hasColumn('news', 'deleted_at')) {
                    $simpleWhere .= ' AND n.deleted_at IS NULL';
                }
                $st2 = $this->pdo->prepare("SELECT COUNT(*) FROM news n WHERE {$simpleWhere}");
                $st2->execute([':cid' => $categoryId]);
                $simpleTotal = (int)($st2->fetchColumn() ?: 0);
                if ($simpleTotal > 0) {
                    $fallback = true;
                    $total = $simpleTotal;
                    $where = $simpleWhere;
                ...
                error_log('[CategoryService] listPublishedNews fallback count error: ' . $e->getMessage());
            }
        }
        ...
        $items = [];
        try {
            // When we are in fallback mode, keep ordering predictable.
            $useOrder = $order;
            if ($fallback && $dateCol === 'id') {
                $useOrder = 'n.id DESC';
            }
            $sql = 'SELECT ' .implode(', ', $select) . " FROM news n WHERE {$where} ORDER BY {$useOrder} LIMIT :lim OFFSET :off";
            $st = $this->pdo->prepare($sql);
            ...
            // Last resort: try without ORDER BY (some hosts are picky when columns are missing)
            try {
                $sql2 = 'SELECT ' .implode(', ', $select) . " FROM news n WHERE {$where} LIMIT :lim OFFSET :off";
                ...
                error_log('[CategoryService] listPublishedNews list fallback error: ' . $e2->getMessage());
            }
        }
