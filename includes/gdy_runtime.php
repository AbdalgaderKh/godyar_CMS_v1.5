<?php
/**
 * Godyar runtime helpers
 * - Safe file write helper (gdy_file_put_contents)
 * - Centralized PHP error logging
 * - Lightweight language + direction detection
 * - Lightweight theme persistence (frontend uses JS, but we expose defaults)
 */
...
if (!function_exists('gdy_file_put_contents')) {
  ...
  function gdy_file_put_contents(string $file, string $data, int $flags = 0): int|false {
    ...
    // Try rename (atomic on same filesystem)
    if (!@rename($tmp, $file)) {
      // Fallback copy
      $ok = @copy($tmp, $file);
      @unlink($tmp);
...
if (!function_exists('gdy_boot_error_logging')) {
  function gdy_boot_error_logging(?string $forcedLogPath = null): void {
    // Do not break if hosting forbids ini_set
    ...
    // Always register handlers (they will write to the configured error_log)
    ...
    set_exception_handler(function ($ex) {
      ...
      error_log($msg . "\n" . $ex->getTraceAsString());
      // Keep default rendering rules (no echo)
    });
  }
}
