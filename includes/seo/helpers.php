 ...
 * SEO helpers (clean).
 *
 * The previous file was corrupted by merges. We provide the functions
 * used across controllers and templates in a stable, CSP-safe way.
 */
...
if (!function_exists('gdy_optimize_html_images')) {
    ...
    {
        // Very light touch: ensure loading=lazy on images without it.
...
if (!function_exists('gdy_extract_faq')) {
    ...
    {
        // Optional: parse simple FAQ blocks if present
        $out = [];
        if ($html === '') return $out;
