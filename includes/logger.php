...
/**
 * logger.php — Simple JSONL logger + error/exception handler.
 *
 * الهدف:
 * - توحيد مسار اللوج (storage/logs)
 * - ضمان إنشاء المجلد تلقائياً
 * - كتابة JSON lines لتسهيل التحليل
 *
 * تفعيل/تعطيل:
 * - GDY_LOG_ENABLED=1 (افتراضي 1)
 * - GDY_LOG_LEVEL=debug|info|warning|error  (افتراضي info)
 */
...
if (!function_exists('gdy_log')) {
    function gdy_log(string $level, string $message, array $context = [], string $file = 'app.log'): void
    {
        ...
        // Prefer gdy_file_put_contents if available, fallback to file_put_contents.
...
if (!function_exists('gdy_register_error_handlers')) {
    ...
    {
        // Ensure PHP internal error_log points somewhere writable
        ...
        set_exception_handler(static function (Throwable $e): void {
            ...
            // A safe minimal response in production
            if (!headers_sent()) {
