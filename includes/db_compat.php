...
if (function_exists('gdy_db_table_exists') === false) {
    /**
 * Check if a table exists (compat).
 *
 * Supports BOTH historical signatures:
 *   - gdy_db_table_exists(PDO $pdo, string $table)
 *   - gdy_db_table_exists(string $table, ?PDO $pdo = null)
 *
 * Also tolerates accidental string/invalid $pdo by falling back to gdy_pdo_safe().
 */
if (!function_exists('gdy_db_table_exists')) {
    function gdy_db_table_exists($a, $b = null): bool
    {
        ...
        // Fallback PDO if missing/invalid.
 ...
 * Legacy alias.
 * Supports:
 *   - db_table_exists(string $table, ?PDO $pdo = null)
 *   - db_table_exists(PDO $pdo, string $table)
 */
