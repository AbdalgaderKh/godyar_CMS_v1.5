...
// ---------------------------------------------------------
// ✅ Language redirects (SEO-friendly) — BEFORE bootstrap
// - /ar  -> /ar/   (same for en/fr)
// - Optional browser-language redirect on homepage (/ -> /{lang}/)
//   Controlled by env: GDY_AUTO_LANG_REDIRECT=1 (default on)
// ---------------------------------------------------------
$__reqUri  = (string)($_SERVER['REQUEST_URI'] ?? '/');
$__reqPath = (string)(parse_url($__reqUri, PHP_URL_PATH) ?: '/');
if ($__reqPath === '') $__reqPath = '/';

// 1) Force trailing slash for language roots: /ar -> /ar/
if (preg_match('~^/(ar|en|fr)$~i', $__reqPath, $__m)) {
    $L = strtolower((string)$__m[1]);
    header('Location: /' . $L . '/', true, 301);
    exit;
}

// 2) Auto redirect homepage based on cookie / browser language
$__auto = getenv('GDY_AUTO_LANG_REDIRECT');
if ($__auto === false || $__auto === '' || $__auto === '1' || strtolower($__auto) === 'true') {
    if ($__reqPath === '/' && empty($_GET['lang'])) {
        ...
        // Only redirect if site uses language prefixes
