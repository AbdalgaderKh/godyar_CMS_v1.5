...
final class NewsExtrasController
{
    ...
    public function tts(): void
    {
        ...
        try {
            ...
            // Extract readable text from DB body (very defensive).
            ...
            // Cache path (outside public). We'll stream file via this endpoint.
            ...
            if (!is_file($outFile) || filesize($outFile) < 1024) {
                // --- Preferred: Google Cloud Text-to-Speech (REST, API Key) ---
                ...
                if ($apiKey !== '') {
                    ...
                    $payload = [
                        'input' => ['text' => mb_substr($text, 0, 4500, 'UTF-8')], // Google limit ~5000 chars
                        'voice' => [
                    ...
                    // Fallback (legacy): exec engines (may be blocked on shared hosting).
                    ...
                    // Keep old logic by delegating to internal helper (if exists in older builds).
    ...
    /**
     * Minimal JSON POST client (cURL preferred).
     * Returns decoded array (or ['error'=>...,'http_code'=>...]).
     */
...
function version(): string
    ...
    public function pushSubscribe(): void
    {
        try {
            ...
            // Ensure table exists (MySQL/PGSQL)
