...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($errorMessage === '') {
        if ($login === '' || $password === '') {
            ...
            try {
                // ⚠️ لا نعتمد على User::login() لأن بعض الإصدارات تستخدم اتصال DB غير مهيّأ
                // مما يسبب: Call to a member function query() on null
                $loggedIn = false;
                    ...
                    if (!$user) {
                        ...
                        if (in_array($status, ['blocked', 'banned'], true)) {
                            ...
                            if ($loggedIn) {
                                ...
                                // ✅ Session markers used across the project (unified)
