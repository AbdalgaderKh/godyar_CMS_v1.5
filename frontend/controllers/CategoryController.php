...
// NOTE: Disabled full-page cache for category pages because stale cached empty pages were being served.
// You can re-enable after confirming category listing works reliably.
$__ttl = 0;
$__pageCacheKey = '';
$__didOutputCache = false;
...
$totalItems = 0;
$items = [];
// NOTE: list cache caused stale "empty" category pages on some hosts (opcache/file cache).
// Disable it for category listings so DB is the source of truth.
$ttl = 0;
...
try {
    ...
    $loader = function () use ($pdo, $category, $perPage, $offset, &$CAT_SQL_ERR) {
            ...
            // Be tolerant across schema variations:
            // - some installs don't have publish_at (ORDER BY would fail)
            // - some installs rely on is_published instead of status only
            // - many installs use soft-delete via deleted_at
            $hasDeletedAt = false;
    ...
    // Bypass caching for categories (see note above)
    ...
    if (is_array($payload)) {
        ...
        // Fallback: if filters or cached list yielded empty but the DB has items for this category,
    // show the latest non-deleted rows to avoid a blank section.
    ...
    // Debug comment when still empty (includes both total rows and published rows)
    if (!$items) {
        try {
...
$newsItems = $items;
// The modern view expects `$news` (not `$newsItems`). Keep both for compatibility.
$news = $items;
$currentPage = $page;
...
// Print debug marker in page source to detect cache/routing issues.
// This is safe for production because it's an HTML comment.
try {
    $dbName = '';
    ...
    // Always override with build marker so you can confirm the deployed version.
