...
<section class="container" style="padding-top:24px;padding-bottom:32px;">
  ...
  <div style="display:grid;grid-template-columns:minmax(0,1fr) 280px;gap:24px;align-items:start;">
    <div>
      <?php if ($news): ?>
        <div class="grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:18px;">
          <?php foreach ($news as $item): $item = is_array($item) ? $item : []; $url = (string)($item['url'] ?? ''); if ($url === '') { $nid = (int)($item['id'] ?? 0); $nslug = trim((string)($item['slug'] ?? '')); $url = $nid > 0 ? ($navBaseUrl . '/news/id/' . $nid) : ($navBaseUrl . '/news/' . rawurlencode($nslug)); } $image = gdy_abs_url($baseUrl, (string)($item['featured_image'] ?? $item['image'] ?? $item['image_path'] ?? $item['thumbnail'] ?? '')); $title = (string)($item['title'] ?? (__('news.untitled', 'خبر بدون عنوان'))); ?>
            <article class="card" style="background:#fff;border:1px solid #e5e7eb;border-radius:16px;overflow:hidden;box-shadow:0 6px 18px rgba(0,0,0,.04)">
      ...
      <?php else: ?>
        <div class="empty" style="padding:24px;border-radius:16px;background:#fff;border:1px dashed #cbd5e1;color:#6b7280"><?= gdy_e(__('category.empty', 'لا توجد أخبار داخل هذا القسم حالياً.')) ?></div>
      <?php endif; ?>
    </div>
    <aside>
      <?php if ($categories): ?>
        <section class="as-block" style="background:#fff;border:1px solid #e5e7eb;border-radius:16px;padding:16px;box-shadow:0 6px 18px rgba(0,0,0,.04)">
          <div class="as-block-hd" style="margin-bottom:12px"><h2 class="as-block-title" style="font-size:18px;margin:0"><?= gdy_e(__('category.sections', 'الأقسام')) ?></h2></div>
          <div class="p-3" style="display:flex;flex-wrap:wrap;gap:10px">
            <?php foreach ($categories as $c): if (!is_array($c)) continue; $slug=(string)($c['slug'] ?? ''); if ($slug==='') continue; $href=$navBaseUrl.'/category/'.rawurlencode($slug); $active=$slug===$catSlug; ?>
              <a class="as-pill<?= $active ? ' is-active' : '' ?>" href="<?= gdy_e($href) ?>" style="display:inline-block;padding:8px 12px;border-radius:999px;border:1px solid #d1d5db;text-decoration:none;color:#111827;<?= $active ? 'background:#111827;color:#fff;border-color:#111827;' : '' ?>"><?= gdy_e((string)gdy_tr('category', (int)($c['id'] ?? 0), 'name', (string)($c['name'] ?? $slug))) ?></a>
            <?php endforeach; ?>
          </div>
        </section>
      <?php endif; ?>
