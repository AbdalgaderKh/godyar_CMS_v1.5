<?php
/*
 * Frontend header (full document start
 * - CSP friendly (no inline JS)
 * - Single source of truth for theme + URLs
 */
...
// Expose settings globally so other partials (theme_head.php, footer, etc.)
// render consistently on every route.
$GLOBALS['site_settings'] = $siteSettings;

$baseUrl = isset($baseUrl) ? (string)$baseUrl : ((string)($siteSettings['base_url'] ?? ''));
$baseUrl = rtrim($baseUrl, '/');

// Root URL used for language-less links
$rootUrl = isset($rootUrl) ? (string)$rootUrl : ($baseUrl ?: '');
$rootUrl = rtrim($rootUrl, '/');

// Navigation base URL (can include language prefix if the router sets it)
...
// Back-compat variables used by legacy templates
$lang = $pageLang;
$dir  = $pageDir;
...
// -------------------------
// Language prefix inference (supports /ar, /en, /fr ...)
// Used for category/archive/contact URLs to prevent subfolder path bugs.
// -------------------------
...
// Prefer navBaseUrl if it already contains /ar|/en|/fr, otherwise build with rootUrl + prefix
...
// -------------------------
// buildLangUrl helper (fix: undefined variable in some deployments)
// -------------------------
if (!isset($buildLangUrl) || !is_callable($buildLangUrl)) {
  $buildLangUrl = function (string $target) use ($__uri, $__rootAbs): string {
    ...
    $path = $path !== '' ? $path : '/';
    // remove existing leading /ar|/en|/fr
...
// إذا كان CSP_STRICT مفعل وnonce غير متاح، حاول تهيئة الهيدر الأمني لضمان عمل JS
...
// Build nonce attribute for external scripts when CSP requires it
...
// Normalize canonical to an absolute URL
if ($canonical_url !== '') {
...
// Theme (single source)
// -------------------------
// NOTE: logic-only patch. Keeps the original header markup intact.
// This fixes: selected theme not applying (frontend stuck on default).
//
// Sources supported:
// 1) DB/site settings keys
// 2) Session/Cookie (optional)
// 3) storage/config/site_theme.php written by admin theme selector

// --- Active theme (single source of truth) ---
// Robust loader: this partial lives in /frontend/views/partials
// Correct relative path to /includes is ../../../includes (NOT ../../../../includes).
if (!function_exists('gdy_get_active_theme')) {
    $candidates = [
        __DIR__ . '/../../../includes/theme_utils.php',
        // legacy/alt layouts (won't hurt):
        __DIR__ . '/../../includes/theme_utils.php',
        __DIR__ . '/../../../../includes/theme_utils.php',
    ];
    ...
    // Absolute fallback: keep frontend working even if the util file is missing.
    if (!function_exists('gdy_get_active_theme')) {
        /** @return string */
        function gdy_get_active_theme($settings = null): string {
            ...
            if (trim((string)$theme) === '') {
                ...
                // from this file location up to public_html
...
// Theme helper may return either a string slug (legacy) or an array {slug,key,path} (new).
$activeThemeInfo = gdy_get_active_theme($siteSettings);

// Normalize the theme slug to prevent values like "Array" or "theme-Array" leaking into HTML.
...
if (is_array($activeThemeInfo)) {
  ...
  // Always build theme class from normalized slug (do NOT trust provided key).
...
// Expose for theme_head.php and any other partials
...
// Logo key can be stored as site.logo (new) or site_logo/logo (legacy)
...
// -------------------------
// Back-compat: some templates call $buildLangUrl(...)
// Ensure it's always defined and callable.
// -------------------------
if (!isset($buildLangUrl) || !is_callable($buildLangUrl)) {
...
<head>
  ...
  <?php
    // Hreflang alternates (helps search engines understand language variants)
  ...
  <?php
    // x-default: same path without language prefix
  ...
  <?php
    // Pagination hints for crawlers (set $meta_prev_url / $meta_next_url in controllers when applicable)
  ...
  <link rel="sitemap" type="application/xml" href="<?= h(($baseUrl ?: '') . '/sitemap.xml') ?>">
...
<?php endif; ?>
  ...
  <div class="gdy-search-overlay" data-search-overlay aria-hidden="true">
    ...
    <div class="gdy-search-overlay__panel" role="dialog" aria-modal="true" aria-label="<?= h($searchLabel ?? __('search.label', 'بحث')) ?>">
...
<?php if (!empty($__breaking['title']) && !empty($__breaking['slug'])): ?>
  <div class="gdy-breaking-ticker" role="region" aria-label="<?= h(__('breaking.bar', 'الأخبار العاجلة')) ?>">
    <div class="container">
