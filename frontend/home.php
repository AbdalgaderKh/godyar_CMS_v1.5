...
// CSP nonce helper for inline scripts (if CSP is active)
$__gdy_nonce = '';
...
// Back-compat base url for templates/partials (avoid undefined warnings)
...
// ==================== سلايدر الواجهة (يدار من لوحة التحكم: admin/slider) ====================
// Table: slider (id, title, subtitle, image_path, link_url, is_active, sort_order, created_at)
$sliderSlides = [];
if ($pdo instanceof PDO) {
    try {
        $sliderTable = 'slider';
...
// Normalize slider image/url (handles relative uploads paths)
...
// بلوك الواجهة الرئيسية (Assahifa layout)
?>
<div class="as-hero">
  <div class="as-container">
    ...
    <?php
      ...
      // Mini cards: keep them as news (better UX) even when slider is managed from admin
    ...
    <div class="as-grid as-grid-hero">
      <?php if(!empty($hero)): ?>
        <?php
          ...
          $heroImg = $heroImgRaw !== '' ? gdy_front_abs_url($_gdy_baseUrl, $heroImgRaw) : ($_gdy_baseUrl.'/assets/images/placeholder.webp');

	        // Build payload for client-side rotation when multiple slides exist.
	        $slidesPayload = [];
	        foreach ((array)$sliderSlides as $it) {
	          ...
	          $im = $p !== '' ? gdy_front_abs_url($_gdy_baseUrl, $p) : ($_gdy_baseUrl.'/assets/images/placeholder.webp');
	          $slidesPayload[] = ['url' => $u, 'img' => $im, 'title' => $t, 'kicker' => $k];
	        }
	        // NOTE: Store slider payload as base64 to avoid any edge-case issues with HTML entity encoding
	        // or CMS/minifiers altering quotes inside attributes. JS decodes and parses it.
        ...
        ?>
	      <article class="as-feature as-feature--slider" data-gdy-hero-slider-b64="<?= $slidesB64 ?>" data-gdy-hero-idx="0" tabindex="0">
	        ...
	        <div class="gdy-hero-ui" aria-label="<?= h(__('Slider controls')) ?>">
      ...
      <div class="as-mini-grid">
        <?php foreach($miniItems as $item): ?>
          <?php
            $u = $buildNewsUrl($item);
            $img = !empty($item['image']) ? $item['image'] : ($_gdy_baseUrl.'/assets/images/placeholder-thumb.jpg');
    ...
    <?php if(!empty($breakingNews)): ?>
      <div class="as-breaking" aria-label="<?= __('Breaking news') ?>">
        <div class="as-breaking-bar">
	          <div class="as-breaking-label"><?= __('t_9c9c9d0d5a', 'عاجل') ?></div>
	...
	(function(){
	  try{
	    ...
	    if (!el) return;
	    var raw = '';
	    // Prefer base64 payload (v9+) and fallback to legacy JSON-in-attribute.
	    ...
	    function setSlide(i, opts){
	      ...
	      var s = slides[i];
	      if (!s) return;
	      // Fade transition (keeps layout stable)
	    ...
	    function start(){
	      ...
	      stop();
	      // Ensure we always render something immediately
	    ...
	    // Keyboard navigation (only when focused within slider)
	    el.addEventListener('keydown', function(ev){
	      try{
	        ...
	        if (k === 'ArrowLeft' || k === 'ArrowRight'){
	          ev.preventDefault();
	          stop();
	          // RTL-aware: ArrowLeft means "next" in RTL.
	    ...
	    (function(){
	      ...
	      el.addEventListener('touchend', function(e){
	        try{
	          ...
	          var rtl = (document.documentElement.getAttribute('dir') || '').toLowerCase() === 'rtl';
	          // Swipe left normally means next; in RTL it means prev.
	          var swipeLeft = dx < 0;
    ...
    <div class="as-grid as-grid-main">
        ...
        <?php if(!empty($latestNews)): ?>
          <div class="as-list">
            <?php foreach($latestNews as $n): ?>
              <?php
                $u = $buildNewsUrl($n);
                $img = !empty($n['image']) ? $n['image'] : ($_gdy_baseUrl.'/assets/images/placeholder-thumb.jpg');
      ...
      <aside class="as-side">
        ...
        <section class="as-block mb-3">
          ...
          <div class="p-3" class="gdy-flex-wrap-gap">
