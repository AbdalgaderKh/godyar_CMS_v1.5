/**
 * Godyar UI Enhancements v4
 * - Reliable dark mode toggle + persistence
 * - Persist theme-* class across language changes
 * - Update meta theme-color based on active theme
 * - Push toast buttons fallback
 */
(function(){
  ...
  function wireModeToggle(){
    // capture to survive any stopPropagation in bundle
  ...
  function persistTheme(){
    try{
      ...
      // Prefer stored theme (selected previously) over whatever the server rendered
      if (stored) {
      ...
      // Mirror to cookie (path=/) so PHP can render the same theme on the homepage/all pages
      try {
        if (current) {
