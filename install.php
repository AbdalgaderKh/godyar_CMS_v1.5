<?php
/**
 * Godyar News Platform - Installer Pro (clean distribution)
 */
...
*{box-sizing:border-box}body{margin:0;background:linear-gradient(180deg,#050b16,#0a1322);color:var(--text);font-family:Tahoma,Arial,sans-serif;padding:28px}.wrap{max-width:1040px;margin:0 auto}.card{background:var(--card);border:1px solid var(--line);border-radius:22px;overflow:hidden;box-shadow:0 35px 90px rgba(0,0,0,.45)}.header{padding:28px 30px;background:linear-gradient(135deg,var(--primary),var(--primary2));display:flex;align-items:center;justify-content:space-between;gap:20px;flex-wrap:wrap}.header h1{margin:0;font-size:34px;line-height:1.2}.header small{display:block;opacity:.95;margin-top:8px;font-size:18px}.version{display:inline-block;padding:10px 18px;border-radius:999px;background:#1e293b;color:#fff;font-weight:800;font-size:18px}.body{padding:24px 28px 30px}.muted{color:var(--muted)}input,select,textarea{width:100%;padding:14px 16px;border-radius:14px;border:1px solid #334155;background:#081120;color:var(--text);font-size:16px}label{display:block;margin:14px 0 8px;font-weight:700}.grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.btns{display:flex;gap:12px;flex-wrap:wrap;margin-top:16px}.btn{background:var(--primary);color:#fff;border:0;border-radius:12px;padding:13px 18px;cursor:pointer;font-weight:800;font-size:15px;text-decoration:none;display:inline-block}.btn.secondary{background:#1e293b}.btn.ok{background:#059669}.err{background:#7f1d1d;border:1px solid #991b1b;color:#fee2e2;padding:14px;border-radius:14px;margin-bottom:14px}.okbox{background:#064e3b;border:1px solid #065f46;color:#d1fae5;padding:14px;border-radius:14px;margin-top:14px}.table{width:100%;border-collapse:collapse}.table th,.table td{border-bottom:1px solid var(--line);padding:12px;text-align:right}.pass{color:#34d399;font-weight:800}.fail{color:#fb7185;font-weight:800}.log{background:#081120;border:1px solid var(--line);border-radius:14px;padding:14px;max-height:280px;overflow:auto;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;white-space:pre-wrap}.progress{margin:14px 0 22px}.bar{height:12px;background:#0b1220;border:1px solid var(--line);border-radius:999px;overflow:hidden}.fill{height:100%;width:<?php echo (int)$progress; ?>%;background:linear-gradient(90deg,var(--primary),var(--primary2));transition:width .25s ease}.steps{display:grid;grid-template-columns:repeat(7,1fr);gap:10px;margin-top:12px}.step-pill{padding:10px 8px;text-align:center;border-radius:12px;border:1px solid var(--line);background:#0b1220;font-size:13px}.step-pill.active{background:#1d4ed8;border-color:#2563eb;color:#fff;font-weight:800}.help{background:#0b1220;border:1px solid var(--line);padding:12px 14px;border-radius:14px;margin-top:14px}.inline{display:flex;gap:10px;align-items:center}.inline input[type=checkbox]{width:auto}@media (max-width:860px){.grid,.steps{grid-template-columns:1fr}.header h1{font-size:26px}}
</style>
</head>
<body><div class="wrap"><div class="card"><div class="header"><div><h1>🚀 مُثبّت Godyar News Platform</h1><small>نسخة نظيفة جاهزة للتثبيت مع فحص اتصال قاعدة البيانات واستيراد آمن</small></div><div class="version">v3.2.0-clean</div></div><div class="body"><div class="muted">الخطوة الحالية: <strong><?php echo h($steps[$step] ?? 'Installer'); ?></strong></div><div class="progress"><div class="bar"><div class="fill"></div></div><div class="steps"><?php foreach ($steps as $idx=>$label): ?><div class="step-pill <?php echo $idx === $step ? 'active' : ''; ?>"><?php echo h($label); ?></div><?php endforeach; ?></div></div><?php if ($errors): ?><div class="err"><strong>حدثت أخطاء:</strong><ul><?php foreach ($errors as $e): ?><li><?php echo h($e); ?></li><?php endforeach; ?></ul></div><?php endif; ?>
