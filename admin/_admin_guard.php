...
// -----------------------------------------------------------------------------
// CSRF: start auto-injector for admin HTML, and enforce tokens on POST (except /admin/api/*)
// -----------------------------------------------------------------------------
try {
...
try {
    ...
    if (!$isApi && (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST')) {
        if (function_exists('verify_csrf_or_throw')) {
            verify_csrf_or_throw();
        } elseif (function_exists('verify_csrf')) {
            if (!verify_csrf()) { throw new RuntimeException('CSRF failed'); }
        }
    }
} catch (Throwable $e) {
    ...
    $_SESSION['admin_flash'] = ['type' => 'danger', 'msg' => 'فشل التحقق الأمني. حدّث الصفحة وحاول مجددًا.'];
...
try {
    if (class_exists('Godyar\\Auth') && method_exists('Godyar\\Auth', 'isLoggedIn')) {
        if (!\Godyar\Auth::isLoggedIn()) {
			if (defined('GDY_ADMIN_JSON') && GDY_ADMIN_JSON) {
				if (!headers_sent()) {
					if (function_exists('gdy_security_log')) { gdy_security_log('admin_unauthorized', ['path'=>($_SERVER['REQUEST_URI'] ?? '')]); }
        ...
        if ($pdo instanceof PDO) {
            ...
            if (!isset($_SESSION['session_version']) || !is_numeric($_SESSION['session_version'])) {
                ...
                if ($sessSv !== $dbSv) {
                    ...
                    if (defined('GDY_ADMIN_JSON') && GDY_ADMIN_JSON) {
                        if (!headers_sent()) if (function_exists('gdy_security_log')) { gdy_security_log('admin_unauthorized', ['path'=>($_SERVER['REQUEST_URI'] ?? '')]); }
				header('Content-Type: application/json; charset=utf-8');
                        http_response_code(401);
