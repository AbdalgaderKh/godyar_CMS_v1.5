...
// This file is meant to be included from admin/settings/index.php.
// If accessed directly, redirect to the settings hub so the full admin layout
// (auth, header, footer, CSRF, etc.) is applied.
...
// Fix: persist frontend theme consistently (DB + storage/config/site_theme.php)
// Also updates legacy keys: front_theme, frontend_theme, front_preset.
...
if (defined('GODYAR_ROOT') && is_dir(GODYAR_ROOT)) {
  ...
  // /admin/settings/theme.php -> go up 2 levels to project root
...
// ---- Read available themes from /assets/css/themes ----
...
// ---- Current theme (prefer DB keys, then config file) ----
$__current = null;
...
// ---- Normalize current to a known choice (if file exists) ----
if ($themes && !in_array($__current, $themes, true)) {
  // If stored as URL or /assets..., normalize to assets/...
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // Accept legacy field names just in case
  ...
  if ($themes && !in_array($sel, $themes, true)) {
    ...
    // 1) DB: update all related keys for compatibility
