...
// Security: optional upgrade key + one-time lock
...
<head>
  <meta charset="utf-8">
  <title>ترقية قاعدة البيانات — Godyar CMS</title>
  ...
  <div class="card">
    ...
    <?php if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['run'])): ?>
      ...
      <pre><?php
        ...
        // Run migrations IN-PROCESS to avoid CLI using old lsphp
        ...
        if (!$pdo) {
            ...
            if (!$ok) {
                ...
                // One-time lock file to prevent accidental reruns
