...
// Base URL resolution
$base = '';
if (defined('ROOT_URL'))      { $base = rtrim((string) ROOT_URL, '/'); }
elseif (defined('BASE_URL'))  { $base = rtrim((string) BASE_URL, '/'); }
elseif (defined('GODYAR_BASE_URL')) { $base = rtrim((string) GODYAR_BASE_URL, '/'); }
...
// Normalize base: strip trailing /admin and trailing /ar|/en|/fr
if ($base !== '') {
    ...
    if (preg_match('~/(admin)$~i', $path)) { $path = preg_replace('~/admin$~i', '', $path); }
    if (preg_match('~/(ar|en|fr)$~i', $path)) { $path = preg_replace('~/(ar|en|fr)$~i', '', $path); }

    if ($scheme && $host) { $base = $scheme . '://' . $host . ($port ? ':' . $port : '') . $path; }
    else { $base = $path; }
}
...
// Admin theme preset (stored as admin.theme). Fallback derive from site_theme.
$adminTheme = 'blue';
...
try {
    if (class_exists('Godyar\\DB') && method_exists('Godyar\\DB', 'pdoOrNull')) {
        ...
        if ($pdo instanceof \PDO) {
            ...
            if ($hasUpdatedAt) { $sql .= ' ORDER BY updated_at DESC'; }
            $sql .= ' LIMIT 1';
...
if (!$__adminThemeFound) {
    ...
    if (!$siteThemePath) {
        ...
        if (is_file($cfg)) {
            ...
            if ($v !== '') { $siteThemePath = $v; }
        }
    }

    $t = strtolower(basename($siteThemePath));
    $name = '';
    if (preg_match('/theme-([a-z0-9_-]+)\\.css/', $t, $mm)) { $name = $mm[1]; }

    $map = [
        'default'=>'default','red'=>'red','blue'=>'blue','green'=>'green','brown'=>'brown',
        'purple'=>'purple','orange'=>'orange','teal'=>'teal','pink'=>'pink',
        'beige'=>'brown','emerald'=>'green','light'=>'default','core'=>'default'
    ];
    if ($name && isset($map[$name])) { $adminTheme = $map[$name]; }
}

// JSON flags for safe embedding
...
<head>
  ...
  <script nonce="<?php echo h($cspNonce); ?>">
    // Expose base URLs for the admin UI
    ...
    // Admin mode (light/dark) persistence
    (function(){
      try {
    ...
    // Patch <svg><use href="#id"></use></svg> to external sprite
    (function(){
      function patch(){
        try {
