...
// توافق بسيط إذا لم تكن دوال الترجمة/اللغة محمّلة من مكان آخر
...
// لو القائمة الديناميكية ناقصة نرجع للثابتة
$dbMenuItemCount = 0;
    ...
    <div class="admin-sidebar__card">
        ...
        <div class="admin-sidebar__search-wrapper">
            <div class="admin-sidebar__search">
                ...
                <div id="sidebarSearchResults" class="admin-sidebar__search-results" role="listbox" aria-label="<?php echo h(__('search_results', 'نتائج البحث')); ?>"></div>
            </div>
        </div>

        <?php if (!$isWriter): ?>
            <div class="admin-sidebar__quick" aria-label="<?php echo h(__('quick_stats', 'إحصاءات سريعة')); ?>">
        ...
        <nav class="admin-sidebar__nav" role="list">
            ...
            <?php else: ?>
                ...
                <div class="admin-sidebar__section" aria-label="<?php echo h(__('نظرة عامة', 'نظرة عامة')); ?>">
                ...
                <div class="admin-sidebar__section" aria-label="<?php echo h(__('المحتوى', 'المحتوى')); ?>">
                ...
                <?php if (!$isWriter): ?>
                    <div class="admin-sidebar__section" aria-label="<?php echo h(__('الإدارة', 'الإدارة')); ?>">
        ...
        <footer class="admin-sidebar__footer">
            ...
            <div class="admin-sidebar__footer-version" style="margin-top:10px;color:rgba(255,255,255,.70);font-size:.78rem;display:flex;justify-content:space-between;gap:8px;flex-wrap:wrap;">
                ...
                <span style="font-weight:800;"><?php echo h(function_exists('gdy_cms_badge') ? gdy_cms_badge() : 'Godyar News Platform v3.0.0-newsroom-core'); ?></span>
            </div>
        </footer>

        <div class="admin-sidebar__section" aria-label="<?php echo h(__('language', 'اللغة')); ?>">
...
:root {
  ...
  --gdy-sidebar-accent: var(--gdy-accent, #38bdf8);
...
.admin-sidebar__card{
  ...
  box-shadow: 0 0 25px rgba(15,23,42,.75);
}
...
.admin-sidebar__search-result-item{
  display:block;
  padding:.45rem .7rem;
  font-size:.8rem;
  color:var(--gdy-sidebar-text);
...
.admin-sidebar__link-card:hover{
  transform:translateY(-1px);
  border-color:color-mix(in srgb, var(--gdy-sidebar-accent) 35%, transparent 65%);
}
.admin-sidebar__link-card.is-active{
  border-color:color-mix(in srgb, var(--gdy-sidebar-accent) 55%, transparent 45%);
  box-shadow:0 0 0 1px color-mix(in srgb, var(--gdy-sidebar-accent) 20%, transparent 80%) inset;
}
...
.admin-sidebar__action-btn--danger{border-color:color-mix(in srgb, var(--gdy-sidebar-danger) 35%, transparent 65%);}
.admin-sidebar__action-btn--danger:hover{border-color:color-mix(in srgb, var(--gdy-sidebar-danger) 65%, transparent 35%);}
</style>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var cards         = Array.from(document.querySelectorAll('.admin-sidebar__link-card'));
  ...
  if (sidebar) {
    ...
    var labels = sidebar.querySelectorAll('.admin-sidebar__link-text, .admin-sidebar__link-label, .admin-sidebar__link-sub');
