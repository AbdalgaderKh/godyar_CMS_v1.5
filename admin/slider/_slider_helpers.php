...
/**
 * Slider module helpers.
 *
 * This CMS historically had more than one slider implementation (sliders/slider).
 * On some installs the `slider` table may not exist.
 *
 * Strategy:
 * - Prefer `slider` table.
 * - Fallback to `sliders` table if it exists.
 * - If neither exists, create `slider` (safe IF NOT EXISTS).
 */
...
if (!function_exists('gdy_slider_table')) {
    ...
    {
        // Prefer modern table
        // Core helper signature is: gdy_db_table_exists(PDO $pdo, string $table)
        ...
        // Create the table (MySQL/MariaDB safe)
        try {
            $pdo->exec(
            ...
            // If table creation fails, keep the module from fataling; it will surface as a SQL error.
...
if (!function_exists('gdy_slider_qt')) {
    /** Quote identifier for dynamic table usage */
